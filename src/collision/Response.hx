...
class Response {
	...
	static function check_collision(packet: CollisionPacket, tris: Array<World.Triangle>) {
		for (tri in tris) {
	...
	static function collide_and_slide(packet: CollisionPacket, gravity: Vec3) {
		...
		// lift to higher surface if we're colliding with something but still grounded...
		if (packet.grounded) {
			final_position.z = Math.max(packet.intersect_point.z, final_position.z);
		}
	...
	public static function update(packet: CollisionPacket) {
